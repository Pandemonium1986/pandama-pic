...
PORTAINER_TOKEN=$(http POST portainer.docker.local/api/auth Username="admin" Password="$PORTAINER_ADMIN_PASSWORD" | jq -r .jwt)

# Renaming endpoint
http PUT portainer.docker.local/api/endpoints/1 \
  "Authorization: Bearer $PORTAINER_TOKEN" \
  Name="Docker Local Unix Socket"

# Teams
http POST portainer.docker.local/api/teams \
  "Authorization: Bearer $PORTAINER_TOKEN" \
  Name="dev"
http POST portainer.docker.local/api/teams \
  "Authorization: Bearer $PORTAINER_TOKEN" \
...
# Users
http POST portainer.docker.local/api/users \
  "Authorization: Bearer $PORTAINER_TOKEN" \
...
# Memberships
http POST portainer.docker.local/api/team_memberships \
  "Authorization: Bearer $PORTAINER_TOKEN" \
...
# Endpoint Access
http PUT portainer.docker.local/api/endpoints/1/access \
  "Authorization: Bearer $PORTAINER_TOKEN" \
  AuthorizedTeams:=[1,2]
